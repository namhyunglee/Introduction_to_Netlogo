...
to setup
 ...
 ;     show ( word "has " stock)
      ...
      foreach stock [ ??1 ->
      set prices lput ( ( 1 + mark-up) * ( item ( position ??1 fruit-and-veg ) fruit-and-veg-prices )) prices
      ]
    ]
  ]
...
ask shoppers with [ not empty? shopping-list ]
  [
    ...
    let purchases filter [ ?1 -> member? ?1 [stock] of stall ] shopping-list

    foreach purchases [ ?1 ->
   set spent spent + item ( position ?1 [ stock ] of stall ) [ prices ] of stall
